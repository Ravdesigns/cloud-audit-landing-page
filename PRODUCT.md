...
**A portrait for the McAfee quote, 2026-09-09.** The page carries a **cut-out** stand-in portrait derived from `~/Downloads/linkedin-sales-solutions-pAtA8xe_iVM-unsplash.jpg`, whose macOS `kMDItemWhereFroms` records it as `https://images.unsplash.com/photo-1560250097-0b93528c311a` — an Unsplash photo published by LinkedIn Sales Solutions, **not a photograph of the person quoted**. It is Unsplash-licensed and free for commercial use, so the licence is not the constraint. The constraint is the caption: a face under "Mahesh Tyagarajan, VP Platform Engineering, McAfee" tells every visitor that the face is his. **It therefore ships with a visible "Placeholder image" badge, and the badge is load-bearing, not decoration.** Removing the badge while the stand-in is in place turns the page into a false statement about a named person at a named company. Replace the image with a cleared photograph of him and the badge goes with it. Check `kMDItemWhereFroms` on any candidate portrait first.

**How the cut-out was made,** since it is worth not redoing by hand: Apple's Vision framework, `VNGenerateForegroundInstanceMaskRequest` plus `generateMaskedImage`, driven by a throwaway Swift binary. That is the same subject-lifting engine behind Copy Subject, and it returns a real anti-aliased alpha matte through hair and glasses. A CSS or gradient mask approximating the subject's edge is explicitly the wrong tool and reads worse than no cut-out at all. The asset ships as WebP with alpha, 42KB against 419KB for the equivalent PNG, which is why `server.cjs` now serves `image/webp`.
...
4. **Form inputs stay at 16px on phones and the labels come up to meet them.** Below 16px, iOS Safari zooms the page when a field takes focus, so the input size is not a free variable. The labels were 12px against a 16px input, which read oddly because the label naming a field was smaller than the placeholder inside it. The step is 13/15 on desktop and 14/16 on phones, and the submit button matches the field rather than sitting under it.
5. **The phone breakpoint is 440px, not 390px.** 390 catches an iPhone 12/13/14 and nothing since: a 393px iPhone 15/16, a 402 Pro and a 430 Pro Max all fell through to the tablet rules, which is why the hero kept its desktop line break and its 51px type on a real phone. Anything phone-specific belongs in the 440px block.
6. **The report deck's height is locked to its tallest card at every width.** On mobile the pointers sit under the deck, so letting it resize per view moved the button that had just been tapped by up to 127px. Each card is measured on its own, not with all three unhidden, because they share one grid cell and measuring them together sizes the row to the flexible chart's minimum.
7. **11px is the floor for functional text, 12px for values and short prose.** The small end of the scale had drifted down to 5.7px on the report masthead, with roughly sixty declarations between 8px and 10px. Weight, tracking and colour carry the label-versus-value distinction that size alone used to carry.
