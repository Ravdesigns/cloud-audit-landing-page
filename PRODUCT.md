...
**A portrait for the McAfee quote, settled 2026-09-09.** Three separate attempts were made to place a face beside Mahesh Tyagarajan's attributed quote using `~/Downloads/linkedin-sales-solutions-pAtA8xe_iVM-unsplash.jpg`. That file's macOS `kMDItemWhereFroms` records its origin as `https://images.unsplash.com/photo-1560250097-0b93528c311a` — an **Unsplash stock portrait published by LinkedIn Sales Solutions**, not a photograph of him. It was not used, and must not be. Two reasons, either sufficient: any unverified face beside a real named person's quote presents a stranger as him to every visitor; and the Unsplash licence excludes using a photo of an identifiable person in a way that implies endorsement, which is exactly what a testimonial does. The `MT` monogram holds the slot and a cleared photograph of him drops into the same markup. **Check `kMDItemWhereFroms` on any candidate portrait before using it.**
...
4. **11px is the floor for functional text, 12px for values and short prose.** The small end of the scale had drifted down to 5.7px on the report masthead, with roughly sixty declarations between 8px and 10px. Weight, tracking and colour carry the label-versus-value distinction that size alone used to carry.
